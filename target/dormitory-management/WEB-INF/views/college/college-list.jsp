...
<%--                            按照条件进行搜索--%>
                            <form class="layui-form layui-col-space5">
                                <div class="layui-inline layui-show-xs-block">
                                    <input type="text" name="collegeid" placeholder="请输入学院编号" autocomplete="off" class="layui-input">
                                </div>
                                <div class="layui-inline layui-show-xs-block">
                                    <input type="text" name="collegename" placeholder="请输入学院名称" autocomplete="off" class="layui-input">
                                </div>
                                <div class="layui-inline layui-show-xs-block">
                                    <select name="collstatus">
    ...
    <script type="text/html" id="statusTpl">
        <input type = "checkbox" name = "collstatus" disabled value = "{{d.collstatus}}" lay-skin = "switch" lay-text = "可用|禁用" lay-filter = "collstatus" {{ d.collstatus == 1 ? 'checked': ''}} >
    </script>
    ...
    <script type="text/javascript">
        layui.use(['table','form'], function() {
            ...
            table.on('toolbar(tableList)',function(obj) {
                if(obj.event=="add"){
                    xadmin.open("学院信息","college/toUpdate",500,300,false);
                }
            });
            /*监听表格内工具栏*/
            table.on('tool(tableList)',function (obj){
                if(obj.event=="update"){
                    xadmin.open("学院信息","college/toUpdate?collegeid="+obj.data.collegeid,500,300,false);
                }
                if(obj.event=="delete"){
                    layer.confirm("确定删除该数据吗？",{icon:3,title:"确定提示"},function (index){
                        $.ajax({
                            url:"college/delete",
                            data:{collegeid:obj.data.collegeid},
                            method:"post",
                            dataType:"json",
                            success(res){
                                if(res.code==1){
            ...
            table.render({
                ...
                url:"college/list",/*远程加载JSON数据*/
                cols:[[/*设置列*/
                    {field: 'collegeid',title:'学院编号'},
                    {field: 'collegename',title: '学院名称'},
                    {field: 'collstatus',title: '状态',templet:'#statusTpl'},
